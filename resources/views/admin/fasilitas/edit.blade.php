    ...
    <div class="card" style="max-width: 800px; margin: 0 auto;">
        <div class="card-header">
            <span class="card-title">
                <i class="fa-solid fa-pen-to-square" style="margin-right: 8px; color: #00d98b;"></i>
                Form Edit Fasilitas: {{ $fasilitas->nama }}
            </span>
        </div>
            ...
            <form action="{{ route('admin.fasilitas.update', $fasilitas->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <select name="jenis" required
                        style="width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); 
                        border-radius: 10px; padding: 10px 14px; font-size: 14px; color: var(--text);">
                        <option value="lapangan" {{ old('jenis', $fasilitas->jenis) == 'lapangan' ? 'selected' : '' }}>🏟️
                            Lapangan</option>
                        <option value="ruang_multimedia"
                            {{ old('jenis', $fasilitas->jenis) == 'ruang_multimedia' ? 'selected' : '' }}>📺 Ruang
                            Multimedia</option>
                        <option value="lab" {{ old('jenis', $fasilitas->jenis) == 'lab' ? 'selected' : '' }}>🔬 Lab
                        </option>
                    </select>
                    @error('jenis')
                        <div style="color: #f87171; font-size: 11px; margin-top: 4px;">{{ $message }}</div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <input type="number" name="harga_per_jam"
                            value="{{ old('harga_per_jam', $fasilitas->harga_per_jam) }}" required min="0"
                            step="5000"
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <input type="number" name="kapasitas" value="{{ old('kapasitas', $fasilitas->kapasitas ?? '') }}"
                            min="1" placeholder="Jumlah maksimal orang"
                            style="flex: 1; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); 
                            border-radius: 10px; padding: 10px 14px; font-size: 14px; color: var(--text);">
                    </div>
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    @if ($fasilitas->foto)
                        <div style="margin-bottom: 12px;">
                            <div style="font-size: 11px; color: var(--text-3); margin-bottom: 6px;">Foto Saat Ini:</div>
                            <img src="{{ Storage::url($fasilitas->foto) }}" alt="{{ $fasilitas->nama }}"
                                style="max-width: 150px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);">
                        </div>
                    @endif
                    ...
                    <div style="border: 2px dashed rgba(255,255,255,0.15); border-radius: 12px; padding: 20px; text-align: center; cursor: pointer;"
                        ...
                        <i class="fa-solid fa-cloud-upload-alt"
                            style="font-size: 32px; color: #00d98b; margin-bottom: 8px;"></i>
                        <p style="font-size: 12px; color: var(--text-3); margin: 0;">
                            Klik untuk ganti foto<br>
                    ...
                    <div id="imagePreview" style="margin-top: 12px; display: none;">
                        <div style="font-size: 11px; color: var(--text-3); margin-bottom: 6px;">Preview Foto Baru:</div>
                        <img id="preview"
                            style="max-width: 150px; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);">
                    </div>
                    @error('foto')
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <textarea name="deskripsi" rows="4"
                        style="width: 100%; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); 
                        border-radius: 10px; padding: 10px 14px; font-size: 14px; color: var(--text); resize: vertical;">{{ old('deskripsi', $fasilitas->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <div style="color: #f87171; font-size: 11px; margin-top: 4px;">{{ $message }}</div>
                    @enderror
                </div>
                    ...
                    <div style="display: flex; gap: 16px;">
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                            <input type="radio" name="status" value="aktif"
                                {{ old('status', $fasilitas->status) == 'aktif' ? 'checked' : '' }}
                                style="accent-color: #00d98b;">
                            <span style="font-size: 13px; color: var(--text);">✅ Aktif</span>
                        </label>
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                            <input type="radio" name="status" value="nonaktif"
                                {{ old('status', $fasilitas->status) == 'nonaktif' ? 'checked' : '' }}
                ...
                {{-- Buttons --}}
                <div
                    style="display: flex; gap: 12px; justify-content: flex-end; border-top: 1px solid rgba(255,255,255,0.06); padding-top: 20px;">
                    <a href="{{ route('admin.fasilitas.index') }}" class="btn btn-outline btn-sm">
                        <i class="fa-solid fa-arrow-left"></i> Batal
                    </a>
                    <button type="submit" class="btn btn-green btn-sm"
                        style="background: #00d98b; border-color: #00d98b; color: #000;">
                        <i class="fa-solid fa-save"></i> Update Fasilitas
                    </button>
                </div>
            </form>
        </div>
