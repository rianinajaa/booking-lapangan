    ...
    <style>
        * { font-family: 'Plus Jakarta Sans', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .panel-right {
            ...
            padding: 44px 44px; overflow-y: auto;
        }
        .panel-left {
            position: fixed; top: 0; left: 0; bottom: 0; right: 420px;
            z-index: 5;
        ...
        label.lbl { display:block; color:rgba(255,255,255,0.65); font-size:13px; font-weight:600; margin-bottom:6px; }
        .btn-masuk-desktop {
            width: 100%; background: #fff; color: #08090f;
            padding: 13px; border-radius: 10px; font-weight: 800;
            font-size: 14.5px; letter-spacing: 0.02em; border: none; cursor: pointer;
            transition: background 0.2s, box-shadow 0.2s, transform 0.1s;
        }
        .btn-masuk-desktop:hover { background: #dcfce7; box-shadow: 0 6px 24px rgba(74,222,128,0.22); }
        .btn-masuk-desktop:active { transform: scale(0.99); }
        .divider-d { display:flex; align-items:center; gap:12px; }
        .divider-d::before,.divider-d::after { content:''; flex:1; height:1px; background:rgba(255,255,255,0.10); }
        ...
        @media (max-width: 640px) {
            ...
            .panel-left, .panel-right, .bg-overlay { display: none !important; }
            ...
            .input-mobile::placeholder { color: rgba(255,255,255,0.28); }
            .input-mobile.err { border-color: rgba(248,113,113,0.55); }

            label.lbl-m { display:block; color:rgba(255,255,255,0.58); font-size:11.5px; font-weight:600; margin-bottom:5px; }

            .btn-masuk-mobile {
                width: 100%; background: #fff; color: #08090f;
                padding: 11px; border-radius: 10px; font-weight: 800;
                font-size: 13.5px; letter-spacing: 0.02em; border: none; cursor: pointer;
                transition: background 0.2s, transform 0.1s;
            }
            .btn-masuk-mobile:active { transform: scale(0.98); }

            @keyframes slideUp { from{opacity:0;transform:translateY(28px)} to{opacity:1;transform:translateY(0)} }
            .mobile-form-sheet { animation: slideUp 0.4s cubic-bezier(.22,.68,0,1.1) both; }
            @keyframes fadeDown { from{opacity:0;transform:translateY(-10px)} to{opacity:1;transform:translateY(0)} }
            .mobile-hero-text { animation: fadeDown 0.45s ease both; animation-delay: 0.1s; }
        }

        /* Sembunyikan mobile di desktop */
        @media (min-width: 641px) {
            .mobile-hero, .mobile-form-sheet { display: none !important; }
        }
    </style>
</head>
<body>
    ...
    <div class="mobile-hero">
        ...
        <div class="mobile-hero-text">
            <p style="color:#4ade80;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.18em;margin-bottom:5px;display:flex;align-items:center;gap:5px;">
                <span style="width:5px;height:5px;background:#4ade80;border-radius:50%;display:inline-block;animation:pulse 2s infinite;"></span>
                Booking Lapangan
            </p>
    ...
    <div class="mobile-form-sheet">
        ...
        <div style="margin-bottom:12px;">
            <p style="color:rgba(255,255,255,0.32);font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.16em;margin-bottom:3px;">BookLap</p>
            <h2 style="color:white;font-size:18px;font-weight:800;line-height:1.2;">Selamat Datang Kembali 👋</h2>
            <p style="color:rgba(255,255,255,0.42);font-size:12px;margin-top:4px;">Masuk untuk mengelola jadwal kamu.</p>
        </div>

        @if ($errors->any())
            <div style="margin-bottom:10px;padding:9px 12px;background:rgba(248,113,113,0.12);border:1px solid rgba(248,113,113,0.25);border-radius:9px;color:#fca5a5;font-size:12px;">
                {{ $errors->first() }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login') }}">
            ...
            <div style="margin-bottom:10px;">
                ...
                <input type="email" name="email" value="{{ old('email') }}"
                    class="input-mobile {{ $errors->has('email') ? 'err' : '' }}"
                    placeholder="nama@perusahaan.com" required autofocus/>
                @error('email')<p style="color:#f87171;font-size:11px;margin-top:4px;">{{ $message }}</p>@enderror
            </div>
            ...
            <div style="margin-bottom:10px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px;">
                    ...
                    <span style="color:#4ade80;font-size:11px;font-weight:600;cursor:pointer;">Lupa Password?</span>
                </div>
                <div style="position:relative;">
                    <input type="password" name="password" id="mpwd"
                        class="input-mobile {{ $errors->has('password') ? 'err' : '' }}"
                        style="padding-right:38px;" placeholder="••••••••" required/>
                    <button type="button" onclick="togglePwd('mpwd','mes','meh')"
                        ...
                        <svg id="mes" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                            <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                        </svg>
                        <svg id="meh" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="display:none;">
            ...
            <div style="display:flex;align-items:center;gap:7px;margin-bottom:14px;">
                <input type="checkbox" name="remember" id="mrem" style="width:13px;height:13px;accent-color:#4ade80;">
                <label for="mrem" style="color:rgba(255,255,255,0.50);font-size:12px;font-weight:400;cursor:pointer;margin:0;">Ingat saya selama 30 hari</label>
            </div>
        ...
        <div style="display:flex;align-items:center;gap:10px;margin:12px 0;">
            ...
            <span style="color:rgba(255,255,255,0.25);font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:0.1em;">atau</span>
            <div style="flex:1;height:1px;background:rgba(255,255,255,0.09);"></div>
        </div>
        ...
        <p style="text-align:center;font-size:12.5px;color:rgba(255,255,255,0.38);">
            Belum punya akun?
            <a href="{{ route('register') }}" style="color:#4ade80;font-weight:700;text-decoration:none;">Daftar sekarang</a>
        </p>
    ...
    <div class="panel-right">
        <div class="fade-in">
            ...
            <p style="color:rgba(255,255,255,0.45);font-size:13.5px;margin-top:8px;margin-bottom:28px;line-height:1.6;">
                Silakan masuk ke akun Anda untuk mengelola jadwal.
            </p>
        </div>

        @if ($errors->any())
            <div class="fade-in d1" style="margin-bottom:16px;padding:12px 14px;background:rgba(248,113,113,0.12);border:1px solid rgba(248,113,113,0.25);border-radius:10px;color:#fca5a5;font-size:13px;">
                {{ $errors->first() }}
            </div>
        @endif
    ...
    <div class="fade-in d2" style="margin-bottom:16px;">
        ...
        <input type="email" name="email" value="{{ old('email') }}"
            ...
            placeholder="nama@perusahaan.com" required autofocus/>
        @error('email')<p style="color:#f87171;font-size:12px;margin-top:6px;">{{ $message }}</p>@enderror
    </div>

    <div class="fade-in d3" style="margin-bottom:16px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            <label class="lbl" style="margin-bottom:0;">Kata Sandi</label>
            <span style="color:#4ade80;font-size:12px;font-weight:600;cursor:pointer;">Lupa Password?</span>
        </div>
        <div style="position:relative;">
            <input type="password" name="password" id="dpwd"
                class="input-glass {{ $errors->has('password') ? 'err' : '' }}"
                style="padding-right:44px;" placeholder="••••••••" required/>
            <button type="button" onclick="togglePwd('dpwd','des','deh')"
                ...
                <svg id="des" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                    <path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                </svg>
                <svg id="deh" width="17" height="17" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" style="display:none;">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/>
                </svg>
            </button>
        </div>
        @error('password')<p style="color:#f87171;font-size:12px;margin-top:6px;">{{ $message }}</p>@enderror
    </div>

    <div class="fade-in d4" style="display:flex;align-items:center;gap:8px;margin-bottom:22px;">
        <input type="checkbox" name="remember" id="drem" style="width:15px;height:15px;accent-color:#4ade80;">
        <label for="drem" class="lbl" style="margin-bottom:0;font-weight:400;font-size:13.5px;cursor:pointer;">Ingat saya selama 30 hari</label>
    </div>
    ...
    <div class="fade-in d5" style="display:flex;align-items:center;gap:10px;margin:18px 0 0 0;">
        ...
        <span style="color:rgba(255,255,255,0.35);font-size:11px;font-weight:600;letter-spacing:1px;">ATAU</span>
        <div style="flex:1;height:1px;background:rgba(255,255,255,0.12);"></div>
    </div>
    ...
    <div class="fade-in d5" style="margin-top:12px;">
        ...
        <p class="fade-in d8" style="text-align:center;font-size:11.5px;color:rgba(255,255,255,0.18);margin-top:32px;">
            &copy; {{ date('Y') }} BookLap Management System.
        </p>
    </div>
